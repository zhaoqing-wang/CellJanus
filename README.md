...
> **Note:** *`hg38.fa.gz` is not required for routine CellJanus runs once `bowtie2_index/GRCh38_noalt_as/` is extracted. You may delete `hg38.fa.gz` to save space if you only use the pre-built index. Keep it if you plan to rebuild Bowtie2 indexes locally.*
...
| `--confidence` (scRNA-seq) | 0.50 | Kraken2 confidence threshold for scRNA-seq mode (recommended range: 0.5–0.7) |
